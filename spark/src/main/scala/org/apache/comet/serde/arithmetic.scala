...
object CometRound extends CometExpressionSerde[Round] {
  ...
  override def convert(
    ...
    r.child.dataType match {
      ...
      case _ =>
        // Float/Double with non-negative scale, and all Decimal types.
        // DataFusion uses IEEE 754 half-up which matches Spark for the vast majority
        // of practical values. `scale` must be Int64 type in DataFusion.
