...
object GeoParquetFunctions {
  ...
  def registerAll(spark: SparkSession): Unit = {
    val serConf = new SerializableConfiguration(spark.sessionState.newHadoopConf())

    spark.udf.register(
      "st_geoparquet_metadata",
      (path: String) => GeoParquetMetadata.readRawJson(new Path(path), serConf.value).orNull)

    spark.udf.register(
      "st_geoparquet_columns",
      (path: String) =>
        GeoParquetMetadata
          .read(new Path(path), serConf.value)
